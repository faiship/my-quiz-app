...
const App: React.FC = () => {
  ...
  const navigateToQuestion = (index: number) => {
    ...
    const existingAnswer = userAnswers.find(
      (a) => a.questionId === shuffledQuestions[index].id
    );
    if (existingAnswer) {
    ...
    const updatedCerts = certs.map((c) => {
      if (c.id === selectedCertId) {
        if (isAdding) {
          return {
            ...c,
            questions: [...c.questions, { ...q, id: Date.now() }],
          };
        }
        return {
          ...c,
          questions: c.questions.map((item) => (item.id === q.id ? q : item)),
        };
      }
      return c;
    });
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] w-full max-w-2xl p-10 shadow-2xl my-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-3xl font-bold text-white tracking-tight">
              {isAddingMode ? "New Question" : "Edit Scenario"}
            </h3>
            <button
              onClick={onCancel}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {formData.options.map((opt, idx) => (
                <div
                  key={idx}
                  className="flex gap-3 items-center bg-slate-950/50 p-2 pr-4 rounded-2xl border border-slate-800"
                >
                  <input
                    type="checkbox"
            ...
            <div className="flex gap-4 pt-6">
              <button
                ...
                className="flex-[2] bg-indigo-600 hover:bg-indigo-500 text-white font-black py-4 rounded-2xl transition-all shadow-xl shadow-indigo-900/40 text-sm uppercase tracking-widest"
              >
                Commit
              </button>
              {!isAddingMode && (
              ...
              <button
                onClick={onCancel}
                className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold py-4 rounded-2xl transition-all text-sm uppercase tracking-widest"
              >
                Discard
              </button>
            </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-950 p-6">
        <div className="max-w-5xl w-full">
          <div className="text-center mb-16">
            <h1 className="text-6xl font-black text-white mb-5 tracking-tightest">
              Certification <span className="text-indigo-500">Center</span>
            </h1>
            <p className="text-slate-400 text-xl font-light">
              Select a path and begin your high-fidelity training session.
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {certs.map((cert) => (
              <div
                key={cert.id}
                className="group relative bg-slate-900 border border-slate-800 rounded-[2.5rem] p-10 hover:border-indigo-500/50 transition-all flex flex-col shadow-2xl overflow-hidden min-h-[400px]"
              >
                <div
                  className="absolute inset-0 opacity-20 group-hover:opacity-30 transition-opacity duration-700 bg-cover bg-center"
                  style={{
                    backgroundImage: `linear-gradient(to bottom, transparent, #0f172a), url(${cert.background})`,
                  }}
                />
                <div className="relative z-10 flex-1">
                  ...
                  <p className="text-slate-400 font-light text-lg mb-10 leading-relaxed max-w-sm">
                    {cert.description}
                  </p>
                </div>
                <div className="flex gap-4 relative z-10">
                  <button
                    onClick={() => startQuiz(cert.id)}
                    className="flex-[3] bg-indigo-600 hover:bg-indigo-500 text-white font-black py-4 rounded-2xl transition-all shadow-xl text-sm uppercase tracking-widest active:scale-95"
                  >
                    Start Quiz
                  </button>
                  <button
                    ...
                    className="flex-1 bg-slate-800/80 backdrop-blur-sm hover:bg-slate-700 text-white rounded-2xl border border-slate-700 transition-all flex items-center justify-center group/btn"
                    title="Review All"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-6 w-6 text-slate-400 group-hover/btn:text-white transition-colors"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
  ...
  if (state === "REVIEW") {
    ...
    const paginatedQuestions = activeCert.questions.slice(
          ...
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <h1 className="text-2xl font-black text-white tracking-tight">
                {activeCert.title} Manager
              </h1>
            </div>
            <button
              onClick={() => setIsAdding(true)}
              className="px-7 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs transition-all uppercase tracking-[0.2em] shadow-xl shadow-indigo-900/30 flex items-center gap-3"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-4 w-4"
        ...
        <main className="flex-1 max-w-5xl w-full mx-auto p-8 space-y-8">
          <div className="flex flex-wrap items-center justify-between gap-6 bg-slate-900/40 p-8 rounded-[2rem] border border-slate-800">
            <div className="flex items-center gap-4">
              ...
              <select
                className="bg-slate-950 border border-slate-800 text-white rounded-xl px-4 py-2 text-sm outline-none focus:border-indigo-500"
                value={itemsPerPage}
                onChange={(e) => {
              ...
              >
                {[10, 25, 50, activeCert.questions.length].map((v) => (
                  <option key={v} value={v}>
                    {v === activeCert.questions.length
                      ? "View All"
                      : `${v} Entries`}
                  </option>
                ))}
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="p-3 rounded-xl bg-slate-800 text-slate-400 hover:text-white disabled:opacity-20 transition-all"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
              ...
              <span className="text-xs font-black text-slate-500 px-5 tabular-nums">
          ...
          <div className="space-y-5">
            {paginatedQuestions.map((q, idx) => {
              ...
              return (
                <div
                  key={q.id}
                  className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-2xl transition-all"
                >
                  <div className="p-8 flex items-start justify-between gap-6">
                    <div className="flex items-start gap-6 flex-1">
                      <span className="w-10 h-10 bg-slate-950 border border-slate-800 rounded-2xl flex items-center justify-center text-xs font-black text-slate-500 mt-1 tabular-nums">
                        ...
                        {q.imageUrl && (
                          <div className="mt-4">
                            <img
                              src={q.imageUrl}
                              alt="Ref"
                              className="h-20 w-auto rounded-xl border border-slate-800 object-cover"
                            />
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="p-3 text-slate-500 hover:text-indigo-400 hover:bg-slate-950 rounded-2xl transition-all"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-5 w-5"
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5M16.5 3.5a2.121 2.121 0 013 3L11.707 15.207a1 1 0 01-.414.293l-3 1a1 1 0 01-1.264-1.264l1-3a1 1 0 01.293-.414L16.5 3.5z"
                          />
                        </svg>
                      </button>
                      <button
                        onClick={() => handleDeleteQuestion(q.id)}
                        className="p-3 text-slate-500 hover:text-rose-400 hover:bg-slate-950 rounded-2xl transition-all"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-5 w-5"
                      ...
                      <button
                        ...
                        className={`p-3 transition-all hover:bg-slate-950 rounded-2xl border border-transparent ${isExpanded ? "text-white rotate-180 bg-slate-950" : "text-slate-500"}`}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-5 w-5"
                    ...
                    <div className="px-8 pb-8 pt-4 border-t border-slate-800 bg-slate-950/40">
                      <div className="space-y-3 mt-4">
                        {q.options.map((opt, i) => (
                          <div
                            key={i}
                            className={`p-5 rounded-2xl border flex items-center justify-between transition-all ${opt.isCorrect ? "bg-emerald-500/10 border-emerald-500/40 text-emerald-400" : "bg-slate-900 border-slate-800 text-slate-500"}`}
                          >
                            <div className="flex items-center gap-4">
                              <span
                                className={`w-8 h-8 rounded-xl flex items-center justify-center text-[10px] font-black ${opt.isCorrect ? "bg-emerald-500 text-slate-950" : "bg-slate-800 text-slate-600"}`}
                              >
                      ...
                      <div className="mt-8 p-7 bg-slate-900 border border-slate-800 rounded-3xl">
                        <span className="block text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-3">
                          Architectural Insight
                        </span>
                        <div className="text-slate-400 text-base leading-relaxed font-light italic">
                          {q.insight}
                        </div>
                      </div>
                    </div>
          ...
          <div className="max-w-5xl mx-auto py-5 flex items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              <button
                ...
                className={`p-3 rounded-2xl border transition-all active:scale-90 ${showMatrix ? "bg-indigo-600 border-indigo-500 text-white shadow-xl shadow-indigo-900/40" : "bg-slate-800 border-slate-700 text-slate-400 hover:text-white"}`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-6 w-6"
            ...
            <button
              ...
              className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
            >
              End Quiz
            </button>
          </div>
          {showMatrix && (
            <div className="max-w-5xl mx-auto pb-8 animate-in slide-in-from-top duration-500">
              <QuestionMatrix />
            </div>
          )}
        </div>
        ...
        <main className="flex-1 max-w-4xl w-full mx-auto p-4 md:p-10 flex flex-col overflow-hidden">
          <div className="bg-slate-900 rounded-[3rem] border border-slate-800 overflow-hidden shadow-2xl flex flex-col flex-1">
            <div className="p-10 md:p-14 overflow-y-auto custom-scrollbar flex-1">
              ...
              <h2 className="text-3xl font-bold text-white leading-tight mb-10 tracking-tight">
                ...
                {currentQuestion.options.map((option, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group w-full text-left p-6 md:p-8 rounded-[1.5rem] border-2 transition-all flex items-start gap-6 ${isSelected ? "border-indigo-500 bg-indigo-500/5" : "border-slate-800 hover:border-slate-700 hover:bg-slate-800/40"} ${isFeedbackShown && option.isCorrect ? "border-emerald-500/50 bg-emerald-500/10" : ""} ${isFeedbackShown && isSelected && !option.isCorrect ? "border-rose-500/50 bg-rose-500/10" : ""}`}
                    >
                      <div
                        className={`flex-shrink-0 w-10 h-10 rounded-xl border-2 flex items-center justify-center transition-all ${isSelected ? "bg-indigo-600 border-indigo-500 text-white shadow-xl shadow-indigo-500/30" : "border-slate-800 text-slate-500"} ${isFeedbackShown && option.isCorrect ? "bg-emerald-600 border-emerald-500 text-white" : ""}`}
                      >
                        {isMultiSelect ? (
                          isSelected ? (
                            <svg
                      ...
                      <span
                        className={`text-lg md:text-xl font-medium leading-relaxed ${isSelected ? "text-white" : "text-slate-400 group-hover:text-slate-300"}`}
                      >
                        {option.text}
                      </span>
                    </button>
              ...
              {isFeedbackShown && (
                <div className="mt-14 p-10 bg-slate-950/60 border border-slate-800 rounded-[2.5rem] shadow-inner animate-in fade-in slide-in-from-bottom-8 duration-700 relative overflow-hidden">
                  ...
                  <div
                    className={`text-slate-300 text-xl leading-relaxed font-light whitespace-pre-line italic transition-all duration-500 ${isExplanationExpanded ? "" : "max-h-40 overflow-hidden line-clamp-4"}`}
                  >
                  ...
                  {currentQuestion.insight.length > 250 && (
                    ...
                    >
                      ...
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className={`h-4 w-4 transition-transform ${isExplanationExpanded ? "rotate-180" : ""}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
            ...
            <div className="p-10 bg-slate-900 border-t border-slate-800 flex items-center justify-between gap-6">
              <button
                ...
                className="px-8 py-5 rounded-2xl font-black bg-slate-800 text-slate-400 hover:text-white disabled:opacity-20 transition-all border border-slate-700 text-xs uppercase tracking-widest active:scale-95 shadow-lg"
              >
                Previous
              </button>
              {!isFeedbackShown ? (
                <button
                  ...
                  className={`px-16 py-5 rounded-2xl font-black transition-all text-xs uppercase tracking-widest active:scale-95 shadow-2xl ${selectedOptions.length > 0 ? "bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-900/50 scale-105" : "bg-slate-800 text-slate-600 cursor-not-allowed border border-slate-700"}`}
                >
                  Verify Selection
                </button>
              ) : (
                <button
                  ...
                  className="px-16 py-5 rounded-2xl font-black bg-white text-slate-950 hover:bg-indigo-50 hover:scale-105 transition-all flex items-center gap-4 text-xs uppercase tracking-widest active:scale-95 shadow-2xl group"
                >
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
        <div className="max-w-xl w-full bg-slate-900 rounded-[4rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.7)] p-14 text-center border border-slate-800 relative overflow-hidden animate-in zoom-in duration-1000">
          ...
          <h2 className="text-5xl font-black text-white mb-3 tracking-tightest">
            Quiz <span className="text-indigo-500">Summary</span>
          </h2>
          <p className="text-slate-500 mb-14 font-light text-lg uppercase tracking-[0.2em]">
            {activeCert.title}
          </p>
          <div className="flex justify-center mb-16">
            <div className="w-56 h-56 relative flex items-center justify-center group">
              <svg
                className="w-full h-full transform -rotate-90 filter drop-shadow-[0_0_20px_rgba(99,102,241,0.3)] transition-all duration-1000 group-hover:scale-110"
                viewBox="0 0 36 36"
              >
                <circle
              ...
              <div className="absolute flex flex-col items-center">
                ...
                <span className="text-[10px] uppercase tracking-[0.4em] text-slate-600 font-black mt-3">
                  Accuracy
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-12">
            <div className="bg-slate-950/50 p-8 rounded-[2.5rem] border border-slate-800 text-left hover:border-slate-700 transition-colors">
            ...
            <div className="bg-slate-950/50 p-8 rounded-[2.5rem] border border-slate-800 text-left hover:border-slate-700 transition-colors">
              ...
              <span
                className={`text-3xl font-black ${score >= 70 ? "text-indigo-400" : "text-rose-500"}`}
              >
                {score >= 70 ? "CERTIFIED" : "RETAKE"}
              </span>
            </div>
          </div>
          <div className="space-y-5">
            <button
              ...
              className="w-full bg-slate-800 hover:bg-slate-700 text-white font-black py-6 rounded-[2rem] border-2 border-slate-700 transition-all flex items-center justify-center gap-4 text-sm uppercase tracking-widest active:scale-95 shadow-xl"
            >
              Audit Scenarios
            </button>
            <button
              onClick={resetSession}
              className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-6 rounded-[2rem] transition-all shadow-[0_20px_40px_-10px_rgba(99,102,241,0.4)] text-sm uppercase tracking-widest active:scale-95 scale-105"
            >
              New Path
            </button>
          </div>
